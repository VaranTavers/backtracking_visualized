...
class Code {
  ...
  ok(k) {
    ...
    this.lineActions = [
      ...
      () => {
        if (colorsNum[t[this.variables.k]] < 0) {
          this.activeLine++;
        } else {
          this.activeLine += 3;
        }
...
function createColorDiv(color, i) {
  ...
  valueElem.id = `db_${i}`;
  valueElem.value = colorsNum[i];
  valueElem.addEventListener('change', (event) => {
    colorsNum[i] = event.target.value;
  });
...
function redrawNums() {
  for (let i = 0; i < n; i++) {
    document.getElementById(`db_${i}`).value = colorsNum[i];
  }
}
...
function startGame() {
  ...
  colorsNum = new Array(possibleColors.length).fill(1);
