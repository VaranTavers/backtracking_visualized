...
function redrawTower(k, guess) {
  ...
  for (let i = k; i >= 0; i--) {
    ...
    block.style.width = `${blockSize[t[i]]}px`;
    block.classList.add('block');
    block.textContent = t[i];
    towerDiv.appendChild(block);
  }
...
class Code {
  ...
  ok(k) {
    ...
    this.lineActions = [
      ...
      () => {
        if (blockSize[t[this.variables.k]] > blockSize[t[this.variables.k - 1]]) {
          this.activeLine++;
        } else {
          this.activeLine += 3;
        }
...
function createColorDiv(color, i) {
  const block = document.createElement('div');
  block.classList.add('color_div');
  block.style['background-color'] = color;

  const indexElem = document.createElement('div');
  indexElem.classList.add('index');
  indexElem.textContent = i;

  block.appendChild(indexElem);

  const valueElem = document.createElement('input');
  valueElem.type = 'number';
  valueElem.id = `size_${i}`;
  valueElem.value = blockSize[i];
  valueElem.addEventListener('change', (event) => {
    blockSize[i] = event.target.value;
  });
...
function prepareColorsDiv() {
  ...
  topElem.textContent = 'Colors: ';

  block.appendChild(topElem);

  const botElem = document.createElement('div');
  botElem.textContent = 'size[] = ';

  block.appendChild(botElem);
...
function redrawNums() {
  for (let i = 0; i < n; i++) {
    document.getElementById(`size_${i}`).value = blockSize[i];
  }
}
...
function startGame() {
  ...
  blockSize = new Array(possibleColors.length).fill(1);
  for (let i = 0; i < blockSize.length; i++) {
    blockSize[i] = (2 * i + 1) * 10;
  }
